...
const metrics = [
  { label: 'Stock sync', value: '98%' },
  { label: 'Invoices', value: '1.2k' },
      ...
      <Container className="relative grid min-h-[calc(100vh-7rem)] items-center gap-12 py-10 lg:grid-cols-[0.96fr_1.04fr] lg:py-10">
        ...
        >
          <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/10 px-4 py-2 text-sm font-bold text-white shadow-soft backdrop-blur">
            ...
            Modern African software company
          </div>
          <h1 className="font-display text-4xl font-extrabold leading-[1.06] tracking-normal text-white sm:text-5xl lg:text-[3.35rem] xl:text-[4rem]">
            Smart software systems for growing businesses.
          </h1>
          <p className="mt-6 max-w-2xl text-lg leading-8 text-white/[0.72] lg:text-[1.15rem]">
            SOMACIT designs MERN platforms, management systems, automation tools, and digital experiences that feel clear, reliable, and ready for daily business.
          </p>
          ...
          >
            <div className="rounded-[24px] bg-somcit-navy p-5 text-white">
              <div className="flex items-center justify-between border-b border-white/10 pb-5">
                <div>
                  <p className="text-sm font-semibold text-white/60">SOMACIT OS</p>
                  <p className="mt-1 font-display text-2xl font-extrabold">Operations overview</p>
                </div>
                <span className="rounded-full bg-somcit-blue px-4 py-2 text-xs font-black">Live</span>
              </div>
              <div className="mt-5 grid gap-4 sm:grid-cols-3">
                {metrics.map((metric) => (
                  <div key={metric.label} className="rounded-2xl bg-white/[0.08] p-4">
                    <p className="text-2xl font-black">{metric.value}</p>
                    <p className="mt-1 text-xs font-semibold text-white/[0.55]">{metric.label}</p>
                  </div>
                ))}
              </div>
              <div className="mt-5 grid gap-4 rounded-2xl bg-white p-4 text-somcit-ink md:grid-cols-[1fr_0.75fr]">
                <div>
                  <div className="mb-4 flex items-center justify-between">
                    <p className="font-black text-somcit-navy">Business automation</p>
                ...
                <div className="rounded-2xl bg-somcit-sky p-4">
                  <p className="text-sm font-bold text-slate-500">Workflow health</p>
